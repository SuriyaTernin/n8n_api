<template>
  <div class="container py-5">
    ...
    <div class="text-center mb-4">
      ...
      <p class="text-muted">สุริยา ถือนิล 67700300</p>
    </div>
  ...
  <table class="table table-hover align-middle text-center">
            ...
            <tbody>
              <tr v-for="(item, index) in products" :key="index">
                ...
                <td class="text-start">{{ item.products_name }}</td> <td>{{ item.qty }}</td> <td class="fw-bold text-success">{{ item.price }}</td> </tr>
            </tbody>
          </table>
        </div>
...
const fetchData = async () => {
  ...
  try {
    ...
    const response = await fetch('http://localhost:5678/webhook/showproduct')
